...
public class ViewNewAccount {
	...
	*/
	...
	// This alert is when any of the information is left blank
	protected static Alert alertUsernameOrPasswordBlankError = new Alert(AlertType.INFORMATION);
	
	// This alert is used should the user enter an invalid username length
	protected static Alert alertUsernameValidityLengthError = new Alert(AlertType.INFORMATION);
	
	// This alert is used should the user enter an integer as the first character of the username
	protected static Alert alertUsernameValidityLeadingCharacterError = new Alert(AlertType.INFORMATION);
	
	// This alert is used should the user use a special character incorrectly in the username
	protected static Alert alertUsernameValiditySpecialCharacterError = new Alert(AlertType.INFORMATION);
	
	// This alert is used should the user enter an invalid password
	protected static Alert alertPasswordValidityError = new Alert(AlertType.INFORMATION);
    ...
    protected static boolean isValidDeadline = false;
	...
	*/
	...
	public static void displayNewAccount(Stage ps, String ic) {
		...
		// *Fetch the invitation code for this user
		isValidDeadline = theDatabase.checkInvitationCode(theInvitationCode);
		
		
		if (theRole.length() == 0 || !isValidDeadline) {// If there is an issue with the invitation code, display a
			alertInvitationCodeIsInvalid.showAndWait();	// dialog box saying that are when it it
			theDatabase.clearOldInvitationCode(emailAddress); // Clear the invitation code entry
			theInvitationCode = "";
	...
	private ViewNewAccount() {
		...
		// If the username or any of the passwords are blank, this alert will tell the user
		alertUsernameOrPasswordBlankError.setTitle("One of the entries has been left blank");
		alertUsernameOrPasswordBlankError.setHeaderText("You must fill in all necessary information.");
		alertUsernameOrPasswordBlankError.setTitle("Correct this issue and try again please.");
		
		// If the username is invalidly created, this alert will tell the user
		alertUsernameValidityLengthError.setTitle("Please create a valid username");
		alertUsernameValidityLengthError.setHeaderText("Usernames must be between 4 and 16 characters. Also, make sure the username does not start with any other character other than characters from A-Z and a-z.");
		alertUsernameValidityLengthError.setContentText("Correct the username and try again.");
		
		// If the username contains a an integer as the leading character, this alert will tell the user
		alertUsernameValidityLeadingCharacterError.setTitle("Please create a valid username");
		alertUsernameValidityLeadingCharacterError.setHeaderText("Usernames must not start with an integer.");
		alertUsernameValidityLeadingCharacterError.setContentText("Correct the username and try again.");
		
		// If the username contains a special character that is not between subsequent characters, this alert will tell the user
		alertUsernameValiditySpecialCharacterError.setTitle("Please create a valid username");
		alertUsernameValiditySpecialCharacterError.setHeaderText("Usernames with special characters must be special characters surrounded by characters from A-Z, a-z, or 0-9. Also, make sure your username does not contain any forbidden characters.");
		alertUsernameValiditySpecialCharacterError.setContentText("Correct the username and try again.");

		// If the password is invalidly created, this alert will tell the user
		alertPasswordValidityError.setTitle("Please create a valid password");
		alertPasswordValidityError.setHeaderText("Passwords need to be 8 to 32 characters long. Passwords must also contain at least one uppercase character, one lowercase character, one number, and a special character from this selection: ~ ` ! @ # $ % ^ & * ( ) _ + { } [ ] | : , . ? /");
		alertPasswordValidityError.setContentText("Correct the password and try again.");
